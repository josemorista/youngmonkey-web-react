...
import staticTiles from '../../../assets/tiles.json';
...
const Cases: FC = () => {
  ...
  const [tiles, setTiles] = useState<Array<ITile>>(staticTiles || []);
  ...
  return (
    ...
    >
      ...
      <Grid style={{ position: 'relative', padding: '10%', paddingTop: '35%', zIndex: 15 }} spacing={10}>
        {sortedTiles?.map((tile, index) => (<Tile ref={index === 0 ? casesGridRef : null} onClick={() => { setOpenTile(tile) }}
          tileImg={'assets/thumbnails/' + tile.img} key={tile._id} {...tile.cols}><Label align='center'><GridItem xs={1} />
            <GridItem justify='center' xs={10}>{tile.descriptions?.ptBr || ''}</GridItem></Label></Tile>))}
      </Grid>
      {openTile ?
        <Modal >
          <Grid align='center' style={{ height: '100%' }}>
            ...
            <GridItem xs={10} justify='center' style={{ position: 'relative' }}>
              ...
              <ClickAway onClose={() => {
                ...
                <video src={'/assets/videos/' + openTile.video} autoPlay controls loop style={{ width: '100%', height: 'auto', padding: '5px', backgroundColor: 'white' }} />
              </ClickAway>
            </GridItem>
          </Grid>
        </Modal>
